...
namespace Prime
{
	namespace Menu
	{
		namespace SubStates
		{
			...
			Settings::Settings(Urho3D::Context* context) :
				SubState(context)
			{
			...
			void Settings::HandlePlayButton(Urho3D::StringHash eventType, Urho3D::VariantMap& eventData)
			{
				SetSubState(new PlayGameSubState(context_));
			}

			void Settings::HandleEditorButton(Urho3D::StringHash eventType, Urho3D::VariantMap& eventData)
			{
				//SetState(new Editor::EditorState(context_));
			}

			void Settings::HandleSettingsButton(Urho3D::StringHash eventType, Urho3D::VariantMap& eventData)
			{
				SetSubState(new Settings(context_));
			}

			void Settings::HandleExitButton(Urho3D::StringHash eventType, Urho3D::VariantMap& eventData)
			{
				SetSubState(new MainMenuSubState(context_));
			}

			void Settings::HandleStatePreStart(Urho3D::StringHash eventType, Urho3D::VariantMap& eventData)
			{
				...
				SubscribeToEvent(_button_Play, Urho3D::E_RELEASED, HANDLER(Settings, HandlePlayButton));
				SubscribeToEvent(_button_Editor, Urho3D::E_RELEASED, HANDLER(Settings, HandleEditorButton));
				SubscribeToEvent(_button_Settings, Urho3D::E_RELEASED, HANDLER(Settings, HandleSettingsButton));
				SubscribeToEvent(_button_Exit, Urho3D::E_RELEASED, HANDLER(Settings, HandleExitButton));
			}

			void Settings::HandleStateStart(Urho3D::StringHash eventType, Urho3D::VariantMap& eventData)
			{
				Urho3D::UI* ui = context_->GetSubsystem<Urho3D::UI>();
				Urho3D::UIElement* root = ui->GetRoot();
				root->AddChild(this);
			...
			void Settings::HandleStateEnd(Urho3D::StringHash eventType, Urho3D::VariantMap& eventData)
			{
				_buttonContainer->SetVisible(false);
			}

			void Settings::HandleStatePostEnd(Urho3D::StringHash eventType, Urho3D::VariantMap& eventData)
			{
				Remove();
			}
		}
